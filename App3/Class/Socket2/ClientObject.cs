            ...
            try
            {
                using (NetworkStream stream = client.GetStream())
                {
                    ...
                    do
                    {
                        ...
                        if (data.Count > 0)
                        {
                            Logger.Instance.WriteToLog(string.Format("Socket sync: {0}.{1}: {2}", System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, "Получено байт " + data.Count));
