...
namespace App3.Class
{
    class Report
    {
        ...
        public void showReport(ReportViewer rep)
        {
            ...
            {
                Logger.Instance.WriteToLog(string.Format("{0}.{1}: {2}", System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name, System.Reflection.MethodBase.GetCurrentMethod().Name, "Результаты запроса не позволяют построить отчет, так как данные не выбраны"));
                Logger.Instance.WriteToLog(sourceSql);
            }
        }
