...
namespace App3.Class
{
    ...
    public class Address
    {
        ...
        public int DistrictId
        {
            get 
            {
                return DataBase.First("select num from regions2map where name = " + district.Q(), "num").ToInt();
            }
        }
        /// <summary>
        ...
        public string District
        {
            ...
            set
            {
                ...
                if (value != "")
                {
                    dCode = DataBase.First(
                        string.Format("SELECT DISTINCT code FROM kladr.districts WHERE fullname = '{0}'", district),
                        "code"
                    ).ToString();
                }
            }
        ...
        private void Load4DB(DataRow row)
        {
            if (row != null)
            {
                Code = row["code"].ToString();
                region = row["region"].ToString();
                district = App3.Class.Static.DBDict.TRegion[row["district"].ToInt()].Item2.ToString();
                locality = row["locality"].ToString();
                street = row["street"].ToString();
                house = row["house"].ToString();
                address = row["address"].ToString();
