		...
		public static class MessagePacker
		{
			...
			public static bool Pack(Message _msg)
			{
				...
				try
				{
					...
					if (num <= 2525269219u)
					{
						if (num <= 1179256059u)
						{
							if (num != 688659595u)
							{
								...
								if (!(type == "MAIN_HEADER_XGUARD_OKOGATE"))
								{
									goto IL_D9A;
								}
						...
						else if (num != 2506469230u)
						{
							if (num != 2525269219u)
							{
								goto IL_D9A;
							}
							if (!(type == "CONTACTID_MSG_OKOGATE"))
							{
								goto IL_D9A;
							}
						...
						else if (!(type == "MESSAGE_AK_OKOGATE"))
						{
							goto IL_D9A;
						}
					}
					else if (num <= 3456202191u)
					{
						if (num != 3067368276u)
						{
							if (num != 3456202191u)
							...
							if (!(type == "DEL_OBJECT_XGUARD_OKOGATE"))
							{
								goto IL_D9A;
							}
						...
						{
							if (!(type == "USER_ACTION_XGUARD_OKOGATE"))
							{
								goto IL_D9A;
							}
					...
					{
						if (!(type == "ADD_OBJECT_XGUARD_OKOGATE"))
						{
							goto IL_D9A;
						}
						...
						XML.SetValue(xmlDocument, "/Envelope/Header/Number", _msg.Get("GID").ToString());
						XML.SetValue(xmlDocument, "/Envelope/Header/DateTime", DateTime.UtcNow.ToString("s"));
						string gUID2 = XMLReader.mdbconnect.GetGUID(_msg.Get("Object").ToString(), "0", "0", "0", "0");
						XML.SetValue(xmlDocument, "/Envelope/Object/ID", gUID2);
						XML.SetValue(xmlDocument, "/Envelope/Object/Number", _msg.Get("Object").ToString());
						...
						for (int j = 0; j < fields2.Count / 4; j++)
						{
							XML.SetValue(xmlDocument, "/Envelope/Object/Zones/Zone", j, "/ID", fields2.GetField(j * 4));
							XML.SetValue(xmlDocument, "/Envelope/Object/Zones/Zone", j, "/Number", fields2.GetField(j * 4));
							XML.SetValue(xmlDocument, "/Envelope/Object/Zones/Zone", j, "/PartNumber", fields2.GetField(j * 4 + 1));
							XML.SetValue(xmlDocument, "/Envelope/Object/Zones/Zone", j, "/Name", fields2.GetField(j * 4 + 2));
							XML.SetValue(xmlDocument, "/Envelope/Object/Zones/Zone", j, "/DeviceTypeName", fields2.GetField(j * 4 + 3));
						}
						_msg.Text = xmlDocument.InnerXml;
						_msg.Content = Encoding.UTF8.GetBytes(_msg.Text);
						bool result = true;
					...
					if (codeIndex >= 0)
					{
						...
						case OKO2.MT.MT56:
						{
							int num6 = (OKO2.OKO2MsgTable[codeIndex].Type - OKO2.MT.MT53) * 8 + 1;
							uint num7 = uint.Parse(_msg.Get("Part").ToString());
							for (int l = 0; l < 8; l++)
							{
								if ((num7 >> l & 1u) != 0u)
								{
			...
			public static bool PackNext(Message _msg)
			{
				if (_msg.Get("PackNext").ToString() == "1")
				{
					...
					{
						string type = _msg.Type;
						if (type != null && (type == "MESSAGE_AK_OKOGATE" || type == "MESSAGE_PULT_OKOGATE"))
						{
							int codeIndex = OKO2.GetCodeIndex(byte.Parse(_msg.Get("Class").ToString()), byte.Parse(_msg.Get("Code").ToString()));
							if (codeIndex >= 0)
							{
								OKO2.MT type2 = OKO2.OKO2MsgTable[codeIndex].Type;
								if (type2 == OKO2.MT.MT23 || type2 == OKO2.MT.MT24)
								{
									int num = (OKO2.OKO2MsgTable[codeIndex].Type - OKO2.MT.MT23) * 8 + 1;
									byte b = byte.Parse(_msg.Get("Part").ToString());
									for (int i = (int)_msg.Get("PackNextParam0") + 1; i < 8; i++)
									{
										if ((b >> i & 1) != 0)
										{
								...
								else
								{
									...
									case OKO2.MT.MT56:
									{
										int num2 = (OKO2.OKO2MsgTable[codeIndex].Type - OKO2.MT.MT53) * 8 + 1;
										byte b3 = byte.Parse(_msg.Get("Part").ToString());
										for (int j = (int)_msg.Get("PackNextParam0") + 1; j < 8; j++)
										{
											if ((b3 >> j & 1) != 0)
											{
												_msg.Set("PackNextParam0", j);
												byte b4 = (byte)(j + num2);
												XmlDocument xmlDocument2 = new XmlDocument();
												xmlDocument2.LoadXml(_msg.Text);
												XML.SetValue(xmlDocument2, "/Envelope/Header/Number", _msg.Get("GID").ToString());
												XML.SetValue(xmlDocument2, "/Envelope/Event/PartNumber", b4.ToString());
												_msg.Text = xmlDocument2.InnerXml;
												_msg.Content = Encoding.UTF8.GetBytes(_msg.Text);
												bool result = true;
												return result;
											}
			...
			public static bool Unpack(Message _msg)
			{
				try
				{
					...
					if (xmlDocument.SelectSingleNode("/Envelope/Header") != null)
					{
						...
						case 2:
						{
							...
							if (codeIndex >= 0)
							{
								...
								case OKO2.MT.MT24:
								{
									OKO2.MT type = ContactID.CIDMsgTable[codeIndex].Type;
									if (type != OKO2.MT.MT20)
									{
										if (type == OKO2.MT.MT21)
										{
											num2 = 0;
										}
									}
								...
								{
									OKO2.MT type = ContactID.CIDMsgTable[codeIndex].Type;
									if (type != OKO2.MT.MT50)
									{
										if (type == OKO2.MT.MT51)
										{
											num2 = 0;
										}
									}
						...
						case 3:
						{
							...
							string text = "";
							int num3 = 0;
							while (num3 < 255 && !(XML.GetValue(xmlDocument, "/Envelope/Object/Parts/Part", num3, "/ID") == ""))
							{
								text = text + ((num3 != 0) ? ";" : "") + XML.GetValue(xmlDocument, "/Envelope/Object/Parts/Part", num3, "/Number");
								text = text + ";" + XML.GetValue(xmlDocument, "/Envelope/Object/Parts/Part", num3, "/Name").Replace(";", ",");
								text = text + ";" + XML.GetValue(xmlDocument, "/Envelope/Object/Parts/Part", num3, "/DeviceTypeName").Replace(";", ",");
								num3++;
							}
							_msg.Set("Parts", text);
							string text2 = "";
							int num4 = 0;
							while (num4 < 255 && !(XML.GetValue(xmlDocument, "/Envelope/Object/Zones/Zone", num4, "/ID") == ""))
							{
								text2 = text2 + ((num4 != 0) ? ";" : "") + XML.GetValue(xmlDocument, "/Envelope/Object/Zones/Zone", num4, "/Number");
								text2 = text2 + ";" + XML.GetValue(xmlDocument, "/Envelope/Object/Zones/Zone", num4, "/PartNumber");
								text2 = text2 + ";" + XML.GetValue(xmlDocument, "/Envelope/Object/Zones/Zone", num4, "/Name").Replace(";", ",");
								text2 = text2 + ";" + XML.GetValue(xmlDocument, "/Envelope/Object/Zones/Zone", num4, "/DeviceTypeName").Replace(";", ",");
								num4++;
							}
						...
						case 4:
						{
							...
							XML.GetValue(xmlDocument, "/Envelope/Object/ID");
							_msg.Set("Object", (ushort)SStr.StrToInt(XML.GetValue(xmlDocument, "/Envelope/Object/Number")));
							bool result = true;
							return result;
						}
					...
					else if (xmlDocument.SelectSingleNode("/Envelope/MainHeader") != null)
					{
			...
			public static int GetPacketSize(byte[] buffer, int size)
			{
				for (int i = 0; i < XMLReader.BytePacketsEnds.Length; i++)
				{
					...
					if (num >= 0)
					{
						return num + XMLReader.BytePacketsEnds[i].Length;
					}
				}
				return 0;
			}
