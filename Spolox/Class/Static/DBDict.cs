...
namespace App3.Class.Static
{
    static class DBDict
    {
        ...
        public static IDictionary<int, IDictionary<int, int>> TObjects;
        ...
        {
            TObjects = new Dictionary<int, IDictionary<int, int>>();
            foreach(var row in DataBase.RowSelect("select region_id, number, osm_id from oko.object"))
            {
                if (!TObjects.ContainsKey(row[0].ToInt()))
                {
                    TObjects[row[0].ToInt()] = new Dictionary<int, int>();
                }
                TObjects[row[0].ToInt()][row[1].ToInt()] = row[2].ToInt();
            }
